...
/*
my SQL 사용자와 DB(Data 저장소)의 관계가 연결 아니다
오라클에서는 사용자에게 defalt Table Space 지정하여
login(접속)을 하면 자동으로 기본 DB 가 연결된다alter
...
-- 일반 사용자 생성
-- MY SQL 은 전통적으로 root 사용자로 접속하여 DB 관리한다
-- 최근추세는 root 사용자는 DB, USER 생성만을 담당하고
-- DB 관리는 일반 사용자등 생성하여 실행하도록 권장한다
-- MySQL은 사용자를 생성할때 어떤 방법으로 접속할 것인지를 명시해야 한다
-- minuk 사용자는 localhost 에서만 mySQL 서버에 접속할 수 있다
DROP USER 'minuk'@'localhost';

CREATE USER 'minuk'@'localhost' identified BY '!Korea8080';
...
새로 등록한 사용자에게 DB 에 접근할 수 있는 권한을 부여하기
minuk@localhost 사용자에게 mydb Schema 에 접근하여
모든 DML 명령을 수행할 수 있는 권한을 부여하겠다.
*/
GRANT ALL PRIVILEGES ON mydb.* TO 'minuk'@'localhost';
