...
대부분의 DBMS 들은 먼저 Database 를 생성하고alter
...
데이터를 INSERT 할때마다 자동으로 Sequence 값이 생성되어alter추가된다
5.x MySQL 에서는 AUTO_INCREMENT 칼럼에 이슈가 있었다
5.x 숫자 type 은 주로 INT 형을 사용하는데 INT 형은 정수 11자리이다
때문에 정수형 11자리 범위를 벗어나는 경우 데이터 INSERT 가 불가능 해지는
문제가 있었다
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0102','이민욱','컴공과',1,'0101000','2');
