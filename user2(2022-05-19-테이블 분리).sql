...
/*
tbl_score 테이블의 초기 설걔 오류로 인하여 과목명이 일반 문자열로 등록되었다
...
-- projection : 칼럼을 원하는 것만, 원하는 수서로 보여달라
...
-- 참조 table ON에서 조건에 맞는 데이터를 조회한다
...
-- 참조 무결성 제약조건
-- foreign key 설정 (FK)
-- A table 과 B table 의 데이터가 참조 무결성이 깨지는게 없게
-- 테이블간에 참조시 - 삽입 이상, 수정 이상, 삭제 이상이 없게
