...
또는 PK 에 해당하는 칼럼을 문자열 32byte 크기로 설정하고
UUID 를 사용하기도 한다
...
-- NEXTVAL 값을 호출(SELECT 등) 할때마다 항상 1증가된 값을 보여준다
SELECT seq_belong.NEXTVAL FROM DUAL;

-- 표준 SQL 에서 간단한 4칙 연산을 수행하는 방법
-- 오라클에선느 SELECT 명령문이 FROM 절이 없이 수행하는 것이 불가능하다
...
-- 2022 학생이 소속된 학과가 D0001 인 것을 확인했다.
...
-- 1. FK 설정하기 앞서 창조무결성 관계를 조회하기
SELECT st_num, st_name, b_seq, b_dcode, d_name, st_grade
FROM tbl_student ST
LEFT JOIN tbl_belong B
ON ST.st_num = B.b_stnum
LEFT JOIN tbl_dept D 
...
-- tbl_student 에 아직 없는 20220004 학번을 추가하려고 했더니
