...
-- PRIMARY KEY 에는 NOT NULL 과 UNIQUE 가 포함되어 있어 따로 적을 필요없다
-- DEFAULT NULL 은 생략해도 된다
...
-- Entity table 의 칼럼과 DOMAIN(type 과 길이) 을 반드시 일치 시켜야 한다
...
-- 전체 칼럼에 데이터 포함하여 INSERT 할때는 칼럼 리스트를 생략할 수 있다.
-- 하지만, 정확히 칼럼에 데이터들이 일치하는 보장이 없다.(안쓰는 것이 좋음)
