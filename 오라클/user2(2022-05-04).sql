...
*/
SELECT sc_stnum, SUM(sc_score) 총점, ROUND(AVG(sc_score)) AS 평균
FROM tbl_score
GROUP BY sc_stnum
...
FROM tbl_score
GROUP BY sc_subject
HAVING sc_subject IN('음악', '국어'); -- 전체 데이터를 다 계산해야해서 너무 낭비

SELECT sc_subject, SUM(sc_score) AS 총점, ROUND(AVG(sc_score), 2 ) 평균
FROM tbl_score
WHERE sc_subject IN('음악','국어') -- 먼저 음악, 국어를 찾고 계산해 낭비없음
GROUP BY sc_subject;

/*
통계함수와 GROUP BY를 사용하여 통계연산을 수행할 때 주의사항
통계연산 결과에 어떤 조건을 부여하여 데이터를 보고자 할때는
WHERE 절이나 HAVING 절에 조건을 부여 할 수 있다.

통계연산이 수행된 결과에 대하여 조건을 부여할 때는
어쩔 수 없이 HAVING 절에 조건을 부여해야 할 것이다
